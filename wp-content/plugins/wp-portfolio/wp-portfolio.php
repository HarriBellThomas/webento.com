...
?&gt;
	...
	<h2><?php echo __('Advanced Features - Disabling pagepix.js inclusion.', 'wp-portfolio'); ?></h2>
	<p><?php echo __('If you\'re not using STW at all, and you want to completely disable <b>pagepix.js</b> from appearing in the header, then copy and paste the following code into your theme\'s <b>functions.php</b>.', 'wp-portfolio'); ?></p>
	...
	if ($websites)
	{
		...
		foreach ($websites as $websitedetails)
		{
			...
			if ($paidAccount || $websitedetails->customthumb) {
				...
				$refreshAction = '<span class="wpp-refresh-disabled">&bull; '.__('Refresh', 'wp-portfolio').'</span>';
			}
...
function WPPortfolio_handleForcedThumbnailRefresh() 
{
	...
	else 
	{
		...
		if (!WPPortfolio_isPaidAccount()) {
			return false; // TODO WPPortfolio_isPaidAccount - can be removed?
		}		
