<?php
/**
 * AccessPress Admin menu/page API
 *
 * @package AccessPress
 */
 ...
 * @since 0.1.0
 */
abstract class AccessPress_Admin {
	...
	public function create( $page_id = '', $menu_ops = array(), $page_ops = array(), $settings_field = '', $default_settings = array() ) {
		...
		$this->page_ops = wp_parse_args( $this->page_ops, array(
			...
			'save_button_text'  => __( 'Save Settings', 'premise' ),
			'reset_button_text' => __( 'Reset Settings', 'premise' ),
			'saved_notice_text' => __( 'Settings saved.', 'premise' ),
			'reset_notice_text' => __( 'Settings reset.', 'premise' )
		) );
		...
		if ( isset( $this->menu_ops['submenu'] ) && ( isset( $this->menu_ops['main_menu'] ) || isset( $this->menu_ops['first_submenu'] ) ) )
			wp_die( sprintf( __( 'You cannot use %s to create two menus in the same subclass. Please use separate subclasses for each menu.', 'premise' ), 'AccessPress_Admin' ) );
 ...
 *
 */
abstract class AccessPress_Admin_Form extends AccessPress_Admin {
	...
	public function admin() {
		...
		<form method="post" action="options.php">
			...
			<h2>
				...
				submit_button( $this->page_ops['save_button_text'], 'button-primary accesspress-h2-button', 'submit', false );
				submit_button( $this->page_ops['reset_button_text'], 'button-secondary accesspress-h2-button', $this->get_field_name( 'reset' ), false, array( 'onclick' => 'return accesspress_confirm(\'' . esc_js( __( 'Are you sure you want to reset?', 'premise' ) ) . '\');' ) );
				?>
			</h2>
			...
			<div class="bottom-buttons">
				...
				submit_button( $this->page_ops['reset_button_text'], 'secondary', $this->get_field_name( 'reset' ), false, array( 'onclick' => 'return accesspress_confirm(\'' . esc_js( __( 'Are you sure you want to reset?', 'premise' ) ) . '\');' ) );
				?>
			</div>
		</form>
		</div>
 ...
 *
 */
abstract class AccessPress_Admin_Boxes extends AccessPress_Admin {
	...
	public function admin() {
		...
		?>
		<div class="wrap accesspress-metaboxes">
		<form method="post" action="options.php">
			...
			<h2>
				...
				echo esc_html( get_admin_page_title() );
				submit_button( $this->page_ops['save_button_text'], 'button-primary accesspress-h2-button', 'submit', false );
				submit_button( $this->page_ops['reset_button_text'], 'button-highlighted accesspress-h2-button', $this->get_field_name( 'reset' ), false, array( 'onclick' => 'return accesspress_confirm(\'' . esc_js( __( 'Are you sure you want to reset?', 'premise' ) ) . '\');' ) );
				?>
			</h2>
			...
			<div class="metabox-holder">
				<div class="postbox-container" style="width: 99%;">
					<?php
					do_action( 'accesspress_admin_before_metaboxes', $this->pagehook );
					do_meta_boxes( $this->pagehook, 'main', null );
					do_action( 'accesspress_admin_after_metaboxes', $this->pagehook );
					?>
				</div>
			</div>
 ...
 *
 */
abstract class AccessPress_Admin_Basic extends AccessPress_Admin {
