...
class Premise_Shortcodes {
	...
	*/
	function aweber_form( $atts, $content = null ) {
		...
		$script = sprintf( '<script type="text/javascript" src="%sforms.aweber.com/form/%d/%d.js"></script>', $scheme, $id % 100, (int)$id );

		ob_start();
		include( PREMISE_VIEWS_DIR . 'misc/aweber-contact-form.php' );
		return ob_get_clean();
	}
	/*
	Constant Contact
	...
	*/
	function mailchimp_form( $atts, $content = null ) {
		...
		$complete = ( isset( $_REQUEST['confirm'] ) && $_REQUEST['confirm'] == '1' );
		$messages = (array)$premise_base->get_transient( '_optin_MailChimpMessages', '_optin_MailChimpCachedMessages' );
		if ( ! $complete ) {

			$ccnumber = $premise_base->get_autonumber( '_optin_MailChimpNumber' );
	...
	*/
	function shortcode_atts( $atts ) {
		...
		$atts = shortcode_atts( array( 'id' => '', 'align' => 'none', 'title' => __( 'Sign Up', 'premise' ) ), $atts );
	...
	*/
	...
	function mailchimp_field( $merge_var ) {
		...
		extract( $merge_var );
		$posted = isset( $_POST['mailchimp'][$tag] ) ? stripslashes_deep( $_POST['mailchimp'][$tag] ) : '';
