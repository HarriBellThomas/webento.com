...
class Premise_Theme {
	...
	function Premise_Theme( $is_home, $page_id ) {
		return $this->__construct( $is_home, $page_id );
	}
	...
	function setup_wp_head() {
		...
		add_action( 'wp_head', array( &$this, 'website_optimizer_scripts' ), 9 );
...
<?php
	...
	function enqueue_theme_scripts_css() {
		...
		if ( file_exists( premise_get_settings_stylesheet_path( $key ) ) ) { 
	...
	function website_optimizer_scripts() {
		...
		if( !empty( $trackingSettings['account-id'] ) && ( $tracking = $premise_base->get_premise_meta( $this->get_post_id(), 'tracking' ) ) && $tracking['enable-gwo'] == 1 ) {
			$isAbTest = $tracking['ab'] == 1;
			$isAbOriginal = $tracking['ab-original'] == 1;
			$isGoal = $tracking['page-type'] == 'goal';
			$testId = $tracking['test-id'];

			if( $isAbTest && $isAbOriginal )
				include( PREMISE_VIEWS_DIR . 'gwo/ab-control.php' );
			elseif ( !$isGoal )
				include( PREMISE_VIEWS_DIR . 'gwo/control.php' );
	...
	function web_optimizer_scripts_after_head() {
		...
		$isGoal = $tracking['page-type'] == 'goal';
		$clicksAsConversions = $tracking['link-click-conversion'];
		$testId = $tracking['test-id'];
		$accountId = $trackingSettings['account-id'];
		...
		if( !$isGoal ) {
			include( PREMISE_VIEWS_DIR . 'gwo/tracking.php' );
			if( $clicksAsConversions )
				include( PREMISE_VIEWS_DIR . 'gwo/click-tracking.php' );
		} else {
			include( PREMISE_VIEWS_DIR . 'gwo/conversion.php' );
		}
	}
	...
	function process_mailing_list_submissions( $wp ) {
		...
		if( isset( $data['mailchimp'] ) && is_array( $data['mailchimp'] ) && isset( $data['mailchimp']['list'] ) && wp_verify_nonce( $data['mailchimp-signup-nonce'], 'mailchimp-signup-'.$data['mailchimp']['list'] ) ) {
			...
			$newvars = array();
			$errors = (array) apply_filters( 'premise_optin_extra_fields_errors', array() );
			foreach( $mv as $data ) {
			...
			wp_redirect( add_query_arg( array( 'confirm' => $confirm ), trailingslashit( home_url( $mc['currenturl'] ) ) ) . '#' . $mc['formkey'] );
			exit();
		}
	}
	function signup_mailchimp_user( $vars, $list ) {
		...
		$result = $mailchimp->listSubscribe( $list, $vars['EMAIL'], $vars );
