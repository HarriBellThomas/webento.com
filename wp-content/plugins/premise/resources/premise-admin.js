...
jQuery(document).ready(function($) {
	$('#premise-sharing-type-simple, #premise-sharing-type-enhanced').bind('change click', function(event) {
		var $this = $(this);
		if($this.is(':checked')) {
			var value = $this.val();
	...
	$('#premise-tracking-enable-gwo').bind('click change', function(event) {
		if($(this).is(':checked')) {
			$('#premise-tracking-enabled-options').show();
		} else {
			$('#premise-tracking-enabled-options').hide();
		}
	...
	$('#premise-main-rewrite').keyup(function(event) {
		$(this).attr('size', $(this).val().length + 1);
	}).keyup();
	
	$('#wpbody-content .wrap > h2:first').append($('#premise-landing-page-type-name').show());
	
	$('#premise-main-rewrite-root').bind('change click', function() {
		if($(this).is(':checked')) {
